...
export class AccountComponent implements OnInit {
    ...
    deleteAccounts() {
        ...
        let req = this.accountService.deleteManyAccounts(this.setOfCheckedItems).pipe(retry(3), catchError(this.handleError))
        if (this.setOfCheckedItems.size == 1) {
            req = this.accountService.deleteAccount(this.setOfCheckedItems).pipe(retry(3), catchError(this.handleError))
        }
